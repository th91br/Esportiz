-- Migration: Authorize receptionist for student payment sync RPC
-- Safe, additive change to role validation logic.
...
BEGIN
    ...
    -- CORREÇÃO: Inclui 'receptionist' no ARRAY de papéis autorizados para permitir que recepcionistas salvem/editem alunos.
    IF NOT (
        public.can_access_organization_financials(v_student.organization_id, ARRAY['owner', 'manager', 'receptionist'])
        OR public.can_access_owner_user_data(v_student.user_id, ARRAY['owner', 'manager', 'receptionist'])
    ) THEN
...
REVOKE ALL ON FUNCTION public.sync_student_unpaid_payments(UUID, BOOLEAN, UUID, INT) FROM PUBLIC, anon;
